...
simi_score = pd.read_csv('./experiments/dtop100_jtop10/gen_ner_nltk_manual_sens_similarity_score_sw_biobert.csv', sep='\t')
            ...
            if senten[1] > 0.60:
                if get_nli(senten) == 'entailment' or get_nli(senten) == 'neutral':
                    ...
                    if senten not in sentences_support:
                        ...
                        journal_dicts.append([senten,senten[1],docs['j_docno']])
...
user_study_df.to_csv('docs__user_study_sen_j_id_055.csv', sep='\t', index=False)
...
user_study_df = pd.read_csv('docs_nltk_user_study_sen_j_id_055.csv', sep='\t')
...
for ii, user_study_qid_data in user_study_qid_df.iterrows():
    ...
    if sens_a:
        ...
        lst_a, lst_b = [i for i in sens_a.split(".") if len(i)>0], text_b.split(".")
        first_text = []
        for i in lst_b:
...
combined_df.to_csv('docs_nltk_user_study_xai_j_id.v3.csv',sep='\t',index=False)
